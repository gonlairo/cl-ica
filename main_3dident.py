...
parser.add_argument("--workers",
                    default=0,
                    type=int,
                    help="Number of workers to use (0=#cpus)")
parser.add_argument("--mode",
                    default="supervised",
                    choices=("supervised", "unsupervised", "test"))
parser.add_argument("--supervised-loss",
                    default="mse",
                    type=str,
                    choices=("mse", "r2"))
parser.add_argument("--unsupervised-loss",
                    default="l2",
                    type=str,
                    choices=("l1", "l2", "l3", "vmf"))
parser.add_argument("--non-periodical-conditional",
                    default="l2",
                    choices=("l1", "l2", "l3"))
parser.add_argument(
    "--sigma",
...
assert not (args.save_model is None and args.save_every
            is not None), "--save-every requires --save-model to be set"

assert not (args.position_only and args.rotation_and_color_only
            ), "Only one of these flags can be set."
assert not (args.position_only and
            (args.non_periodic_rotation_and_color or args.no_spotlight_color
             or args.no_spotlight_position))

assert not (args.box_constraint is not None
            and args.sphere_constraint is not None)
...
def save_model(directory, model_name, total_loss_values,
               linear_disentanglement_scores,
               permutation_disentanglement_scores):
    # loss_path = os.path.join(directory, "loss.npy")
    # linear_disentanglement_score_path = os.path.join(
    #     directory, "linear_disentanglement_score.npy")
    # permutation_disentanglement_scores_path = os.path.join(
    #     directory, "permutation_disentanglement_scores.npy")

    model_path = os.path.join(directory, model_name)
    metrics = {
        "model_state_dict": f.state_dict(),
        "total_loss_values": total_loss_values,
        "linear_disentanglement_scores": linear_disentanglement_scores,
        "permutation_disentanglement_scores":
        permutation_disentanglement_scores
    }
...
def setup_latent_space(args, n_objects=1):
    n_color_and_rotation_variables = (
        n_objects * (4 + (0 if args.no_spotlight_color else 1) +
                     (0 if args.no_spotlight_position else 1)) + 1)
    ...
    if args.non_periodic_rotation_and_color:
        rotation_and_color_space = latent_spaces.LatentSpace(
            spaces.NBoxSpace(n_objects *
                             (4 + (0 if args.no_spotlight_color else 1) +
                              (0 if args.no_spotlight_position else 1) + 1)),
            lambda space, size, device: space.uniform(size, device=device),
            lambda space, z, size, device="cpu": non_periodical_conditional(
                space, z, sigma=sigma, size=size, device=device),
        ...
        color_space = latent_spaces.LatentSpace(
            spaces.NBoxSpace(n_objects *
                             (1 + (0 if args.no_spotlight_color else 1)) + 1),
            lambda space, size, device: space.uniform(size, device=device),
            lambda space, z, size, device="cpu": non_periodical_conditional(
                space, z, sigma=sigma, size=size, device=device),
        )
        ...
        #NOTE: going here
        rotation_and_color_space = latent_spaces.LatentSpace(
            spaces.NSphereSpace(n_color_and_rotation_variables),
            lambda space, size, device: space.uniform(size, device=device),
...
def setup_f(args, n_non_angular_latents, n_angular_latents):
    ...
    if args.box_constraint is not None:
        non_periodic_rescale_layer = layers.SoftclipLayer(
            n=n_non_angular_latents,
            fixed_abs_bound=args.box_constraint == "fix")
    else:
    ...
    if args.position_only:
        ...
        if args.non_periodic_rotation_and_color:
            ...
            #NOTE: going here
            rescaling = layers.Lambda(lambda x: torch.cat(
                (
...
def train_unsupervised(args, train_loader):
    ...
    if args.unsupervised_loss == "l2":
        nonspherical_loss = losses.LpSimCLRLoss(p=2,
                                                tau=1.0,
                                                simclr_compatibility_mode=True,
                                                pow=True)
    elif args.unsupervised_loss == "l1":
        nonspherical_loss = losses.LpSimCLRLoss(p=1,
                                                tau=1.0,
                                                simclr_compatibility_mode=True,
                                                pow=True)
    elif args.unsupervised_loss == "l3":
        nonspherical_loss = losses.LpSimCLRLoss(p=3,
                                                tau=1.0,
                                                simclr_compatibility_mode=True,
                                                pow=True)
    ...
    def train_step(data, loss, optimizer):
        ...
        return (
            ...
            None  #unpack_item_list(losses_value),
        )
    ...
    for global_step in tqdm(range(n_steps)):
        ...
        if global_step % n_log_steps == 0 or global_step == n_steps:
            ...
            (
                ...
                mse,
                linear_fit_mse,
            ) = evaluate(args, f, train_iterator,
                         evaluate_permutation_disentanglement)
        else:
        ...
        if args.save_every is not None:
            if global_step // args.save_every != last_save_at_step // args.save_every:
                model_name = f"checkpoint_{global_step}.pth"
                save_model(directory, model_name, total_loss_values,
                           linear_disentanglement_scores,
                           permutation_disentanglement_scores)
                last_save_at_step = global_step

    return total_loss_values, linear_disentanglement_scores, permutation_disentanglement_scores
...
def evaluate(args,
    ...
    with torch.no_grad():
        for batch_idx in range(args.n_eval_samples // args.batch_size):
            ...
            if no_pairs:
                ...
                batch_z_disentanglement = batch_z_disentanglement.to("cuda")
                batch_x_disentanglement = batch_x_disentanglement.to("cuda")

            if args.dummy_mixing:
                batch_x_disentanglement = g(
                    batch_z_disentanglement.to(device)).cpu()

            if args.identity_mixing_and_solution:
                batch_h_z_disentanglement = batch_z_disentanglement
            else:
                batch_h_z_disentanglement = (
                    f(batch_x_disentanglement).detach().to(
                        batch_z_disentanglement.device))
    ...
    if len(z_disentanglement) > 0:
        ...
        ) = disentanglement_utils.linear_disentanglement(z_disentanglement,
                                                         h_z_disentanglement,
                                                         mode="r2",
        ...
        if not args.identity_solution:
            mse = (mse_distance(
                z_disentanglement,
                h_z_disentanglement).mean(0).detach().cpu().numpy())
        else:
            mse = np.inf
...
latent_space, n_non_angular_latents, n_angular_latents = setup_latent_space(
    args)
...
if args.dummy_mixing or args.identity_mixing_and_solution:
    ...
    dataset_kwargs = dict(transform=transforms.Compose([
        transforms.ToTensor(),
        transforms.Normalize(mean=[0.3292, 0.3278, 0.3215],
                             std=[0.0778, 0.0776, 0.0771]),
    ]))
...
if args.non_periodic_rotation_and_color:
    ...
    if args.no_spotlight_position:
        latent_dimensions_to_use = [
            it for it in latent_dimensions_to_use if it != 6
        ]
    if args.no_spotlight_color:
        latent_dimensions_to_use = [
            it for it in latent_dimensions_to_use if it != 8
        ]
else:
    if args.position_only:
...
if args.mode == "supervised" or args.mode == "unsupervised":
    ...
    test_dataset = SequentialThreeDIdentDataset(args.offline_dataset,
                                                **dataset_kwargs)
